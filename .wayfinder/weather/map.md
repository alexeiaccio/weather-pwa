...
- Data-layer architecture mapped onto Solid + Effect (module shape, caching shape, refresh cadence) — sharpens now that W1 fixed the Open-Meteo contract. W3 sharpened the **place-store** slice of it: a single-pin IndexedDB store, geolocation bootstrap, search-to-pin flow, search empty state — graduates as its own ticket alongside the caching/refresh slice once W5 and W6 land.
