...
- [W4 · Screen content cut](tickets/W4-screen-content-cut.md) — **MVP perimeter: hero + scrubbing hourly curve + 10-day + Metric quartet (Feels Like, Wind, Humidity, Pressure)**, severe-weather banner out, one static column. 10-day rows keep W2 anatomy (precip %, color-encoded range bar, dot at current temp on Today). UV/Visibility/Sunrise/AQ + full grid + alerts stay post-v1 stretch.
- [W7 · Data-layer architecture on Solid + Effect](tickets/W7-data-layer-architecture.md) — **Mirror Spectre: Effect `Context.Service` facade + Schema decoding + @effect/platform-browser IndexedDB; Solid consumes via thin `use-*` hooks + resources; no query lib.** Single-pin place table + forecast-cache table keyed by place+date with TTLs 10min/1h/6h; SW (prompt-register) precaches shell only, payload lives in IndexedDB; static Wrangler Worker + ASSETS binding → `weather.accio.blue`; metric units only in v1. Full module map + dependency sketch in the ticket — the build recipe's blueprint.

> **Status reached 2026-08-19 — every decision ticket is closed; the build recipe is unambiguous. Writing the app is the downstream effort.**

## Not yet specified

- Sky/background gradient animation ambition (static in the W5 prototype) — decided during the build, once the stack is up; the gradient-sky recipe is already captured (W2).
- Stretch goals (RainViewer radar map, saved-city search/place list, alerts via NWS/MeteoAlarm, full metric grid): whether/how they appear — post-v1, re-grilled with the human when the MVP ships.
